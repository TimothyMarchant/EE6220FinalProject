...
ovs-ofctl add-flow s1 priority=500,in_port=$MiddleboxPort,ip,$Camera1,actions=normal
...
ovs-ofctl add-flow s1 priority=600,tp_dst=$ImageServerPort,actions=output:$HighBandWidthPort
ovs-ofctl add-flow s1 priority=600,tp_dst=$TextDataPort,actions=output:$LowBandWidthPort
