#!/bin/bash
###This script runs all flows for S1 corresponding to middlebox 1.
#define IP addresses
...
#ARP rules; these were a pain to deal with.
#camera 1
...
ovs-ofctl add-flow s1 priority=600,in_port=$MiddleboxPort,dl_type=0x800,nw_proto=6,tp_dst=$ImageServerPort,actions=output:$HighBandWidthPort
ovs-ofctl add-flow s1 priority=600,in_port=$MiddleboxPort,dl_type=0x800,nw_proto=6,tp_dst=$TextDataPort,actions=output:$LowBandWidthPort
...
ovs-ofctl add-flow s1 priority=700,in_port=$MiddleboxPort,dl_type=0x800,nw_proto=6,tp_dst=$TextDataPort,ip,ip_dst=$EmergencyCenterIP,actions=output:$LowBandWidthPort
...
ovs-ofctl add-flow s1 priority=50,ip,ip_dst=$EmergencyCenterIP,actions=output:$HighBandWidthPort
ovs-ofctl add-flow s1 priority=50,icmp,ip_dst=$DataCenterIP,actions=output:$HighBandWidthPort
ovs-ofctl add-flow s1 priority=50,ip,ip_dst=$DataCenterIP,actions=output:$HighBandWidthPort
#Emergency AP
