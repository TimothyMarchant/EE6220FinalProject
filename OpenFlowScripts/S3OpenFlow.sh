...
ovs-ofctl add-flow s3 priority=300,ip,ip_src=$EmergencyCenterIP,ip_dst=$Middlebox1,actions=output:2
ovs-ofctl add-flow s3 priority=300,ip,ip_src=$EmergencyCenterIP,ip_dst=$Middlebox2,actions=output:4
#ICMP
