...
Unfortunately Mininet doesn't like large networks (also it does very badly with cycles, even with proper rules) so results will vary.  There is some automation that is controlled by the middleboxes.  It switches the lane that data flows to the emergency center.
